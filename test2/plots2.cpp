...
// Data collection at every iteration:
	if (st==1){
		...
		FILE * fid;
		fid = fopen("plots.cvs","w");
		fprintf(fid," time \t qd1 \t qd2 \t qd3 \t qd4 \t qd5 \t qd6 \t qd7 \t\t time \t qdl \t qdr \t\t time \t q1 \t q2 \t q3 \t q4 \t q5 \t q6 \t q7 \t\t time \t qll \t qrr \t\t time \t x \t y \t z \t\t time \t roll \t pitch \t yaw \t\t time \t xc \t yc \t zc \t\t time \t rollc \t pitchc \t yawc \t\t time \t detJoa \t detJo \n");
		for (j=0; j<timep.RowNo(); j++){
			fprintf(fid," %g \t %g \t %g \t %g \t %g \t %g \t %g \t %g \t\t %g \t %g \t %g \t\t %g \t %g \t %g \t %g \t %g \t %g \t %g \t %g \t\t %g \t %g \t %g \t\t %g \t %g \t %g \t %g \t\t %g \t %g \t %g \t %g \t\t %g \t %g \t %g \t %g \t\t %g \t %g \t %g \t %g \t\t %g \t %g \t %g\n", timep(j,0) , qd1(j,0) , qd2(j,0) , qd3(j,0) , qd4(j,0) , qd5(j,0) , qd6(j,0) , qd7(j,0), timep(j,0) , qdl(j,0) , qdr(j,0), timep(j,0) , q1(j,0) , q2(j,0) , q3(j,0) , q4(j,0) , q5(j,0) , q6(j,0) , q7(j,0), timep(j,0) , qll(j,0) , qrr(j,0), timep(j,0) , x(j,0) , y(j,0), z(j,0), timep(j,0) , roll(j,0) , pitch(j,0), yaw(j,0), timep(j,0) , xc(j,0) , yc(j,0), zc(j,0), timep(j,0) , rollc(j,0) , pitchc(j,0), yawc(j,0), timep(j,0) , detJoap(j,0) , detJop(j,0) );
		}
		fclose(fid);
