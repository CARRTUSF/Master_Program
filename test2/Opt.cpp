...
Matrix WMRA_Opt(int i, float JLA, float JLO, Matrix Jo, float detJo, Matrix dq, Matrix dx, float dt, Matrix q)
{
	...
	FILE * fid_jla;
	fid_jla = fopen("JLA.txt","a");

	double inf = std::numeric_limits<double>::infinity();
	int WCA, wo, ko, j, k;
	Matrix pinvJo(7,7);
	Matrix dH(7,1), dH_temp(7,1);
	...
	if (i==0){
		...
		dH.Null(7,1);
		fprintf(fid_jla,"%f\t", q(3,0)*180/PI);
		if (JLA==1){
			for (j=0; j<7; j++){
				dH_temp(j,0)=abs(-0.25*pow((qmax(0,j)-qmin(0,j)),2)*(2*q(j,0)-qmax(0,j)-qmin(0,j))/(pow((qmax(0,j)-q(j,0)),2)*pow((q(j,0)-qmin(0,j)),2)));
				if (j==3)
					fprintf(fid_jla,"%f\t", dH_temp(j,0));
				// Re-defining the weight in case the joint is moving away from it's limit or the joint limit was exceeded:
				if (dH_temp(j,0) < dH_prev(j,0) && q(j,0) < qmax(0,j) && q(j,0) > qmin(0,j)){
					dH(j,0)=0;
				}
				else if (dH_temp(j,0) >= dH_prev(j,0) && q(j,0) < qmax(0,j) && q(j,0) > qmin(0,j)){
					dH(j,0)=dH_temp(j,0);
				}
				else if (dH_temp(j,0) <= dH_prev(j,0) && (q(j,0) >= qmax(0,j) || q(j,0) <= qmin(0,j))){
					dH(j,0)=inf;
